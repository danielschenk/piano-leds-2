...
#include "../Mock/MockProcessingBlock.hpp"
 ...
 */
class ProcessingBlockContainerTest : public LoggingTest
{
  public:
    ...
    {
        ON_CALL(*redSource, execute(_, _)).WillByDefault(Invoke(addRed));
        ON_CALL(*greenSource, execute(_, _)).WillByDefault(Invoke(addGreen));
        ON_CALL(*valueDoubler, execute(_, _)).WillByDefault(Invoke(doubleValue));
        ON_CALL(*valueDoubler, mode()).WillByDefault(Return(ProcessingBlock::Mode::overwriting));
    }
